...
namespace Lab6.Controllers
{
    public class PostsController : Controller
    {
        ...
        public IActionResult Edit(Guid id)
        {
            ...
            if (post.AuthorId != userId && !HttpContext.User.IsInRole("Admin"))
                return Unauthorized();

            var categories = _dbContext.ForumCategories.ToList();
            return View(new PostEditingViewModel
            {
        ...
        public IActionResult Edit(PostEditingViewModel model)
        {
            if (ModelState.IsValid is false) return View(model);

            var post = _dbContext.Posts.FirstOrDefault(x => x.Id == model.Id);
            if (post is null) return NotFound();
        ...
        public class PostEditingViewModel
        {
