...
using Lab6.Data;
using Lab6.Models;
using Lab6.ViewModels.Folders;
using File = Lab6.Models.File;
...
namespace Lab6.Controllers
{
    public class FoldersController : Controller
    {
        ...
        public IActionResult Create(FolderCreatingViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var folder = new Folder
                {
                    ...
                    StorageType = FileStorageType.Folder
                };
        ...
        public IActionResult Upload(Guid folderId,[FromForm] [Required] IFormFile file)
        {
            if (ModelState.IsValid)
            {
                ...
                _dbContext.Files.Add(new File
                {
                    ...
                    StorageType = GetFileType(file.FileName)
                });
                _dbContext.SaveChanges();
        ...
        private FileStorageType GetFileType(string path)
        {
            string ext = Path.GetExtension(path);
            switch (ext)
            {
                case ".exe": return FileStorageType.Application;
                ...
                case ".txt":
                    return FileStorageType.TextFile;

                case ".xlsx":
                case ".xls":
                    return FileStorageType.Excel;

                case ".docx":
                case ".doc":
                    return FileStorageType.Word;

                default: return FileStorageType.File;
            }
        }
    }
}